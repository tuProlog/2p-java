...
public class TestPrologParser {
    ...
    private static SingletonTermContext parseTerm(String string) {
    ...
    private static SingletonExpressionContext parseExpression(String string) {
        final PrologParser parser = createParser(string);
        parser.addErrorListener(new BaseErrorListener() {
            @Override
            public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line, int charPositionInLine, String msg, RecognitionException e) {
    ...
    public void testInteger() {
        assertionOn(
                parseTerm("1"),
                SingletonTermContext::term,
                t -> t.isNum && !t.isStruct && !t.isExpr && !t.isList && !t.isVar
        ).andThenAssert(
                TermContext::number,
                n -> n.isInt && !n.isReal
        ).andThenAssert(
                NumberContext::integer,
